...
public class HaarWavelet {
	//Pointeiros Importantes para a estrutura toda
	public static int nInteractions;
	public static int[][] finalArray;
	
	//Ponteiros apenas para o uso interno
	...
	public HaarWavelet(int[][] compressedData, int interactions) {
		...
		//no fim, deixa o array no ponteiro universal e só pescar depois.
	...
	public HaarWavelet(float[][] rawData, String nInteractionsManipulado) {
		...
		nInteractions = i;
		//Ver maneira de juntar de uma forma recursiva 
				if (x.size() > 0) {
	...
	public static void main(String[] args) {
		...
		*/
		//Fazer metodo de Transformacao completa
		/*
	...
	*/
	...
	private static float[][] transformation(float[][] rawData) {
		//Que é a juncao de horizontal e vertical transformation
	...
	private static float[][] inversion(float[][] rawData) {
		//Se a transformacao comecou com horizontal e terminou com vertical, o inverso é mesmo vertical e depois horizontal.
	...
	*/
	private static float[][] returnItFlat(float rawData[][], int lengthOriginal) {
		...
		if (lengthOriginal < countGCDNumber(rawData.length, 0)) {
			...
			if (lengthOriginal == 0) {
				...
				/*
				System.out.println("Primeiro Level de Transformation:");
		...
		//Transforma o primeiro bloco apenas
		/*
		...
		*/
		...
		//Cria novo array com o novo bloco e adiciona ao antigo
	...
	private static float[][] optimizeHaarWaveletTransforms(float[][] rawData) {
		...
		int i=0;
		//Bug no suposto ultimo GCD capturado, ele falha. Saltei para o controle por numero de iteracoes.
...
private static float[][] revertHaarWaveletTransforms(float rawData[][]) {			
			if (nInteractions == 1) {
				...
				//Precisaremos de escalar a montanha em base de iteractions, somente para obter os firstblocks todos juntos neh.
				...
				/*
				//Try to print all in
				System.out.println("Printando ja no inverso in "+haarwavelado.length);
				for (int q=0; q<z.size(); q++)
					System.out.println(Arrays.deepToString(z.get(q)));
				System.out.println();
				
				//Pegar o ultimo, fazer inverse. Juntar ao anterior, inverse. até 0, inverse. Devolver.
